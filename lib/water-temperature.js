...
const caches = {
	...
	nearbyStations: lruCache({
		max: 100,
		maxAge: 3600 * 1000
	}),
	noaaStations: lruCache({
		max: 1,
		maxAge: 3600 * 24 * 1000
	})
};
...
async function fetchNOAATemperatureStation(id) {
	...
	url.searchParams.set('application', process.env.APPLICATION);
...
async function fetchNOAATemperatureStations(near) {
	const url = new URL('http://tidesandcurrents.noaa.gov/mdapi/v0.6/webapi/stations.json');
...
export async function getWaterTemperaturesNear(near, limit) {
	...
	return Promise.all(limitedStations.map(async (station) => {
